...
class LocallyWeightedRegression:
    ...
    def __init__(self, data, transposed=False, tau=0.5):
        """
        ...
        def w(i):
            return [
                math.e
                ** (-((np.linalg.norm(self.x_data[i] - x_j)) ** 2) / (2 * tau**2))
                for x_j in self.x_data
            ]
        ...
        def get_coeffs_i(i):
            return (
                np.linalg.pinv(np.transpose(self.x_data) @ W[i] @ self.x_data)
    ...
    def f(self, x):
        ...
        def gauss(centre, x, sigma=1):
            return math.e ** (-((centre - x) ** 2) / (2 * sigma**2))

        summed = 0
        summed_gauss = sum(
            [gauss(self.centres[index], x) for index in range(len(self.coeffs))]
        )
        for index, coeff in enumerate(self.coeffs):
            summed += (
                gauss(self.centres[index], x)
                / summed_gauss
