...
def simple_uniform(
    ...
    *args,
    ...
    rho (int): noise scaling factor on y-values rho \in (0,1] -> Think of this as the
        amplitude of the output data
    *args @toDo: 2 lists, 1 for x the other for error parameters
    Returns:
        x (np array): x-values
    ...
    scale = rho * (max_target - min_target)
    ...
    if distr_eps[0:5] is not "Heter":
        ...
        stretch = np.random.default_rng().choice(
            [-1, 1], N
        )  # vector of -1's and 1's to fuzz in both directions
        fuzz = err * stretch
    else:
        fuzz = None
